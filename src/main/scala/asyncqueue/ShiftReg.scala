...
// Similar to the chisel3 ShiftRegister but allows the user to suggest a
...
object AbstractPipelineReg {
  def apply [T <: chisel3.Data](gen: => AbstractPipelineReg, in: T, name: Option[String] = None): T = {
...
object AsyncResetShiftReg {
  def apply [T <: chisel3.Data](in: T, depth: Int, init: Int  = 0, name: Option[String] = None): T =
    AbstractPipelineReg(new AsyncResetShiftReg(in.getWidth, depth, init), in, name)

  def apply [T <: chisel3.Data](in: T, depth: Int, name: Option[String]): T =
    apply(in, depth, 0, name)

  def apply [T <: chisel3.Data](in: T, depth: Int, init: T, name: Option[String]): T =
    apply(in, depth, init.litValue.toInt, name)

  def apply [T <: chisel3.Data](in: T, depth: Int, init: T): T =
    apply (in, depth, init.litValue.toInt, None)
}

// Note that it is important to override "name" in order to ensure that the chisel3 dedup does
...
object SynchronizerShiftReg {
  def apply [T <: chisel3.Data](in: T, sync: Int = 3, name: Option[String] = None): T = {
    if (sync == 0) in
...
object SyncResetSynchronizerShiftReg {
  def apply [T <: chisel3.Data](in: T, sync: Int = 3, init: T, name: Option[String] = None): T =
